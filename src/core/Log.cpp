...
void Log::Init() noexcept {
	...
	sinks.emplace_back( make_shared<spdlog::sinks::stdout_color_sink_mt>() );

	logger_core_ = make_shared<spdlog::logger>( "OCTAVE", sinks.begin(), sinks.end() );
	spdlog::register_logger( logger_core_ );
	logger_core_->set_level(spdlog::level::trace);
	logger_core_->flush_on(spdlog::level::trace);

	logger_client_ = make_shared<spdlog::logger>( "CLIENT", sinks.begin(), sinks.end() );
